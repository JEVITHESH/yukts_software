...
def extract_agent_attributes(span, agent, user_input: str):
    ...
    span.set_attribute("yukta.agent.system_prompt", getattr(agent.system_prompt, "prompt_text", ""))
...
def extract_llm_attributes(span, client, messages: List[Dict], response: Any):
    ...
    if hasattr(response, "get_cache_info"):
        ...
        if cache_info:
            span.set_attribute("yukta.cache.cached_tokens", int(cache_info.get("cached_tokens", 0)))
            span.set_attribute("yukta.cache.hit_rate", float(cache_info.get("cache_hit_rate", 0.0)))
